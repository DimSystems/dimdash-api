...
async function startSystem() {
    ...
    client.login(config.token).then(() => {
        ...
        console.error("(!) "+err+"\n An error occured while loading the servers. Please fix before merging it into the production branch.");
        process.exit(1);
      }).catch((err) => {
          console.error("(!) "+err+"\n An error occured while logging in to discord. Please fix before merging it into the production branch.");
          process.exit(1);
      })
